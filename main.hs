...
gameFinished m = elem True $ map (elem E) m
...
playingWith board player = do
   if gameFinished board then do
      putStrLn ("Ganaron: " ++ otherPlayer player)
   else do
      i <- getLine
      j <- getLine
      let pos = Position (read i :: Int) (read j :: Int)
      playingWith $ updateBoard board (Position (read i :: Int) (read j :: Int)) otherPlayer player
